...
FILE *fp[4];
fp[0]=fopen("bifSEIRSnSEIstable_SIRnSI_tauB_p001_alph_p0_dB_p99.dat","w");
fp[1]=fopen("bifSEIRSnSEIunstable_SIRnSI_tauB_p001_alph_p0_dB_p99.dat","w");
...
sp.PIM=5000;//recruitment rate of uninfected mosquitoes (per day)-- variable parameter
sp.muB=0.001;//1/1000.0;// 1/sp.muB is average life span of a bird (in days)
sp.dB=0.005;//0.005; //WNV-induced death rate of birds (fraction per day)
...
sp.tauB=0.001;//001428;//recovery rate for birds
...
sp.Corrfactr=fe;
for(sp.R0d=1.1;sp.R0d>=0.4;sp.R0d-=0.0005)
{
sp.R02=sp.R0d;
...
while(il<=50)
{
y0[1]=sp.PIM0/sp.muM*drand48();
y0[2]=sp.PIM0/sp.muM*drand48();
y0[3]=sp.PIM0/sp.muM*drand48();
y0[4]=sp.PIB/sp.muB*drand48();
y0[5]=sp.PIB/sp.muB*drand48();
y0[6]=sp.PIB/sp.muB*drand48();
y0[7]=sp.PIB/sp.muB*drand48();
...
if(sp.lamMH>=0 && y0[1]>=0 && y0[2]>=0 && y0[3]>=0 && y0[4]>=0 && y0[5]>=0 && y0[6]>=0 && y0[7]>=0 && y0[2]>0.541 || y0[2]<0.000001 && sp.R02<1)
{
fprintf(fp[0],"%lf %lf %lf %lf %lf %lf %lf\n",sp.PIM0,sp.R02,sp.lamMH,y0[1],y0[2],y0[3],y0[4]);
il++;
}
if(sp.lamMH>=0 && y0[1]>=0 && y0[2]>=0 && y0[3]>=0 && y0[4]>=0 && y0[5]>=0 && y0[6]>=0 && y0[7]>=0 && y0[2]>0.541 && sp.R02>1)
{
fprintf(fp[0],"%lf %lf %lf %lf %lf %lf %lf\n",sp.PIM0,sp.R02,sp.lamMH,y0[1],y0[2],y0[3],y0[4]);
il++;
}
